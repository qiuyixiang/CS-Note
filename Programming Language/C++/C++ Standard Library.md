...
in header _<exception.h>
```c++
class exception
```
Provides consistent interface to handle errors through the throw expression.

All exceptions generated by the standard library inherit from _std::exception_.
...
The _std::allocator_ class template is the default Allocator used by all standard library containers if no user-specified allocator is provided. The default allocator is stateless, that is, all instances of the given allocator are interchangeable, compare equal and can deallocate memory allocated by any other instance of the same allocator type.
...
| allocate        | allocate uninitialized storage              |
| deallocate      | deallocate the storage                      |
| max_size        | return the lagest supported allocation size |
| construct       | construct an object in allocated storage    |
| destroy         | destructs an object in allocated storage    |

General Utility In the Container :
```c++
std::allocator<Tp> My_allocator;

My_allocator.allocate(n); // malloc
My_allocator.construct(...) // call constructor of the object

My_allocator.destroy(...) // call destructor of the object
My_allocator.deallocate(n); // free
```


## Smart Pointer
There are three different smart pointers in C++ !
...
The Structure of the implementation Shared_ptr
![](../../_IMG/PL/Snipaste_2024-06-04_11-13-07.png)
